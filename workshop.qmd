...
Set the configuration and working directory:

```{python}
%env UNIONAI_CONFIG=/root/.unionai/config.yaml
...
- We're using the `openai_api_key` secret that we made earlier to use GPT4.
...
Why does this take a lot longer than the GPT4 RAG workflow?
...
- The GPT4 workflow simply does an API call on a Union CPU resource.
